...
The `FileUpload` object allows you to upload or download files to
and from a server.
...
The `FileUpload` object provides a way to upload files to a remote
server using a non multi-part HTTP request.  Both HTTP and HTTPS
...
success callback.  If an error occurs, a `FileUploadError` object is
...
- __errorCallback__: A callback that executes if an error occurs retrieving the `Metadata`. Invoked with a `FileUploadError` object. _(Function)_
...
- __errorCallback__: A callback that executes if an error occurs when retrieving the `Metadata`. Invoked with a `FileUploadError` object. _(Function)_
...
Aborts an in-progress transfer. The onerror callback is passed a FileUploadError object which has an error code of FileUploadError.ABORT_ERR.
...
__Example__
    ...
    fileUpload.download(...); // or fileUpload.upload(...);
